...
class AuthGuard(
    ...
    fun requireAgendaStaff(agendaId: UUID): Usuario {
        val agenda = agendaRepository.findByIdOrNull(agendaId) ?: throw NotFoundException("Agenda no encontrada")
        return requireProfesionalOrAssignedAssistant(agenda.profesional.id!!)
    }
    ...
    fun requireTurnoParticipant(turnoId: UUID): Usuario {
        ...
        turno.cliente?.let { cliente ->
            if (usuario.perfilCliente?.id == cliente.id) return usuario
        }
